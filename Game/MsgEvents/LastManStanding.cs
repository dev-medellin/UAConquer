...
namespace TheChosenProject.Game.MsgEvents
{
    public class LastManStanding
    {
        ...
        {
            return EventMap.Values.Where(p => p.Player.DynamicID == DynamicID && p.InLastManStanding).ToArray();
        }
        ...
        private void Execute()
        {
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                foreach (Client.GameClient c in _chrz)
                {
                    c.InLastManStanding = false;
                    c.Teleport(429, 378, 1002);
                    c.Player.SendString(stream, MsgStringPacket.StringID.Effect, true, "end_task");
                    ...
                    using (var conn = new MySql.Data.MySqlClient.MySqlConnection(ConnectionString))
                    {
                            ...
                            , conn))
                        {
                            ...
                            try
                            {
                                using (var cmdd = new MySql.Data.MySqlClient.MySqlCommand("Delete from cq_events where type=1", conn))
                            ...
                            cmd.Parameters.AddWithValue("@EntityID", c.Player.UID);
                            cmd.Parameters.AddWithValue("@name", c.Player.Name);
        ...
        public static void OnDead(Client.GameClient Attacked, ref uint Damage)
        {
