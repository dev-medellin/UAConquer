...
namespace TheChosenProject.Game.MsgMonster
{
    public class MobCollection
    {
        ...
        public MonsterRole Add2(MonsterFamily Famili, bool RemoveOnDead = false, uint dinamicid = 0,
    bool justone = false, bool spawnhere = false, Mobs.Base @base = null)
        {
            if (DMap == null)
                ReadMap();
            MonsterRole monsterr = null;
            int Max = 1;
            int count = (int)((Famili.Boss > 0) ? 1 : (int)(Math.Max(1, (int)Famili.SpawnCount) * (Multiple * Max)));
            if (justone)
                count = Math.Max(1, (int)Famili.SpawnCount);
            if (Famili.Settings == MonsterSettings.Messenger || Famili.SpawnCount == 1)
                count = 1;

            if (DMap.ID == 1001)
                count = Famili.SpawnCount;
            if (Famili.ID == 61190 || Famili.ID == 3143 || Famili.ID == 3146 || Famili.ID == 3149 || Famili.ID == 31569 || Famili.ID == 3102 || Famili.ID == 8500)
                count = Famili.SpawnCount;
            ...
            for (int x = 0; x < count; x++)
            {
                ...
                else
                {
                    Mob.X = Famili.SpawnX;
                    Mob.Y = Famili.SpawnY;
                    Mob.RespawnX = Famili.SpawnX;
                    Mob.RespawnY = Famili.SpawnY;
                }
                Mob.Map = DMap.ID;
        ...
        public static void TryObtainSpawnXY(Role.GameMap DMap, ushort x, ushort y, ushort x_max, ushort y_max, out ushort X, out ushort Y)
        {
            ushort x2 = (ushort)(x + x_max);
            ushort y2 = (ushort)(y + y_max);

            X = (ushort)Program.GetRandom.Next(x, x2);
            Y = (ushort)Program.GetRandom.Next(y, y2);
