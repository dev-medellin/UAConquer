...
namespace TheChosenProject.Game.MsgServer.AttackHandler
{
    public class DragonWhirl
    {
        public unsafe static void Execute(Client.GameClient user, InteractQuery Attack, ServerSockets.Packet stream, Dictionary<ushort, Database.MagicType.Magic> DBSpells)
        {
            ...
            if (Distance >10)
            {
                user.SendSysMesage("Out of range.", MsgMessage.ChatMode.TopLeft);
                return;
            }
            if (CheckAttack.CanUseSpell.Verified(Attack, user, DBSpells, out ClientSpell, out DBSpell))
            {
                ...
                {
                    case (ushort)Role.Flags.SpellID.DragonWhirl:
                        {
                            ...
                            Attack.Damage = (ushort)Role.Flags.SpellID.DragonWhirl;
