    ...
    public class Riding
    {
        public unsafe static void Execute(Client.GameClient user, InteractQuery Attack, ServerSockets.Packet stream, Dictionary<ushort, Database.MagicType.Magic> DBSpells)
        {
            ...
            if (CheckAttack.CanUseSpell.Verified(Attack, user, DBSpells, out ClientSpell, out DBSpell))
            {
                ...
                if (user.Player.Map == 1860 || user.Player.Map == 1005 || user.Player.Map == 2071 || user.Player.Map == 1764 || user.Player.Map == 1858 || user.Player.Map == 8881 || user.Player.Map == 8880 || user.Player.Map == 700 || user.Player.Map == 1767 || MsgTournaments.MsgSchedules.CurrentTournament.InTournament(user))
                {
