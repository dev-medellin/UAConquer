...
namespace TheChosenProject.Game.MsgServer.AttackHandler
{
    public class Transform
    {
        ...
        {
            if (user.Player.ContainFlag(MsgUpdate.Flags.Fly) || user.Player.ContainFlag(MsgUpdate.Flags.Ride) || user.Player.Map == 1005 || user.Player.Map == 700 || user.Player.Map == 1767 || user.Player.Map == 1036)
            {
            ...
            {
                var events = Program.Events.Find(e => e.EventTitle == user.EventBase.EventTitle);
                if (events != null)
                {
                    if (events.InTournament(user))
                    {
                        user.SendSysMesage("You cant use this skill right now !");
                        return;
                    }
