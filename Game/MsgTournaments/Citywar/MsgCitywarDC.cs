...
namespace TheChosenProject.Game.MsgTournaments
{
    public class CoordinateDC
    {
        ...
        public ushort Y;
        public uint Map;
        public CoordinateDC(uint Map, ushort X, ushort Y)
        {
            this.Map = Map;
            this.X = X;
            this.Y = Y;
    ...
    public class MsgCitywarDC
    {

        public bool SendInvitation = false;
        public class CitywarDCScrore
        {
        ...
        public ConcurrentDictionary<uint, CitywarDCScrore> ScoreList;
        public CitywarDCScrore Winner;
        public MsgCitywarDC()
        {
            Proces = ProcesType.Dead;
            Furnitures = new Dictionary<Role.SobNpc.StaticMesh, Role.SobNpc>();
            ScoreList = new ConcurrentDictionary<uint, CitywarDCScrore>();
            Winner = new CitywarDCScrore() { Name = "None", Score = 100, GuildID = 0 };
        }

        public unsafe void CreateFurnitures()
        {

            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Database.Server.ServerMaps[19894].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 834));
        ...
        public void HidePole()
        {
            foreach (var pole in Furnitures.Values)
            {
                if (!CoordsPole.ContainsKey(pole.UID))
                    CoordsPole.Add(pole.UID, new coords() { x = pole.X, y = pole.Y }); pole.X = 0;
                pole.Y = 0;
                using (var rec = new ServerSockets.RecycledPacket())
                {
                    var stream = rec.GetStream();
        ...
        public bool Join(GameClient user, ServerSockets.Packet stream)
        {
            if (Proces == ProcesType.Alive)
            {
                ...
                ushort y;
                y = 0;
                Database.Server.ServerMaps[19894].GetRandCoord(ref x, ref y);
                user.Teleport(x, y, 19894, 0);
                return true;
            }
            return false;
        }
        ...
        internal unsafe void SendMapPacket(ServerSockets.Packet packet)
        {
            foreach (var client in Database.Server.GamePoll.Values)
            {
                if (client.Player.Map == 19894 || client.Player.Map == 6001)
                {
                    client.Send(packet);
                }
            }
        }
        internal unsafe void CompleteEndPoleDominationDC()
        {
            ...
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                if (Winner.Name != "None" && Winner.Score != 100)
                    msg = "Congratulations to " + Winner.Name + ", they've won the DesertCity Pole with a score of " + Winner.Score.ToString();
                else msg = "DesertCity Pole has ended with no winner.";
        ...
        internal unsafe void Start()
        {
            ...
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                ScoreList.Clear();
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("DesertCity Pole war in CityWars NPC has started!", MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.System).GetArray(stream));
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("DesertCity Pole war in CityWars NPC has started!", MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.TopLeft).GetArray(stream));
            }
        }
        ...
        internal unsafe void FinishRound()
        {
            ...
            {
                var stream = rec.GetStream();
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("Congratulations to " + Winner.Name + ", they've won the DesertCity Pole round with a score of " + Winner.Score.ToString() + ""
        ...
        internal unsafe void Began()
        {
            if (Proces == ProcesType.Idle)
            {
                ...
                {
                    var stream = rec.GetStream();
                    Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("DesertCity Pole has began!", MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.System).GetArray(stream));
                    //Program.DiscordEventsAPI.Enqueue("DesertCity CityWar Started");
                }
            }
        }
        internal void UpdateScore(Role.Player client, uint Damage)
        {
            ...
            if (Proces == ProcesType.Alive)
            {
                ...
                {
                    ScoreList.TryAdd(client.GuildID, new CitywarDCScrore() { GuildID = client.MyGuild.Info.GuildID, Name = client.MyGuild.GuildName, Score = Damage });
                }
                else
                {
