...
using static TheChosenProject.Game.MsgTournaments.MsgCitywarPC;
...
namespace TheChosenProject.Game.MsgTournaments
{
    ...
    public class MsgCitywarPC
    {

        public bool SendInvitation = false;
        public class CitywarPCScrore
        {
        ...
        public ConcurrentDictionary<uint, CitywarPCScrore> ScoreList;
        public ConcurrentDictionary<uint, Scoreboard> Scoreboards = new ConcurrentDictionary<uint, Scoreboard>();
        public CitywarPCScrore Winner;

        public MsgCitywarPC()
        {
            Proces = ProcesType.Dead;
            Furnitures = new Dictionary<Role.SobNpc.StaticMesh, Role.SobNpc>();
            ScoreList = new ConcurrentDictionary<uint, CitywarPCScrore>();
            Winner = new CitywarPCScrore() { Name = "None", Score = 100, GuildID = 0 };
        }
        ...
        public bool Join(GameClient user, ServerSockets.Packet stream)
        {
            if (Proces == ProcesType.Alive)
            {
                ...
                if (rnd < 25)
                    user.Teleport(215, 336, 19892);
                else if (rnd < 50)
                    user.Teleport(275, 328, 19892);
                else if (rnd < 75)
                    user.Teleport(285, 263, 19892);
                else
                    user.Teleport(285, 263, 19892);
                return true;
            }
        ...
        public unsafe void CreateFurnitures()
        {
            ...
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Database.Server.ServerMaps[19892].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 832));
        ...
        internal unsafe void SendMapPacket(ServerSockets.Packet packet)
        {
            foreach (var client in Database.Server.GamePoll.Values)
            {
                if (client.Player.Map == 19892 || client.Player.Map == 6001)
                {
                    client.Send(packet);
                }
            }
        ...
        internal unsafe void Start()
        {
            ...
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                ScoreList.Clear();
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("PhoenixCastle Pole war in CityWars NPC has started!", MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.System).GetArray(stream));
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("PhoenixCastle Pole war in CityWars NPC has started!", MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.TopLeft).GetArray(stream));
            }
        }
        ...
        internal unsafe void FinishRound()
        {
            ...
            {
                var stream = rec.GetStream();
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("Congratulations to " + Winner.Name + ", they've won the PhoenixCastle Pole round with a score of " + Winner.Score.ToString() + ""
        ...
        internal unsafe void Began()
        {
            if (Proces == ProcesType.Idle)
            {
                ...
                {
                    var stream = rec.GetStream();
                    Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("PhoenixCastle Pole has began!", MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.System).GetArray(stream));
                    //Program.DiscordEventsAPI.Enqueue("PC CityWar Started");
                }
            }
        }
        internal void UpdateScore(Role.Player client, uint Damage)
        {
            ...
            if (Proces == ProcesType.Alive)
            {
                ...
                {
                    ScoreList.TryAdd(client.GuildID, new CitywarPCScrore() { GuildID = client.MyGuild.Info.GuildID, Name = client.MyGuild.GuildName, Score = Damage });
                }
                else
                {
        ...
        internal unsafe void CompleteEndPoleDominationPC()
        {
            SendInvitation = false;
            ShuffleScores();
            Proces = ProcesType.Dead;
            ScoreList.Clear();
            foreach (var client in Database.Server.ServerMaps[19892].Values)
            {
            ...
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                if (Winner.Name != "None" && Winner.Score != 100)
                    msg = "Congratulations to " + Winner.Name + ", they've won the PhoenixCastle Pole with a score of " + Winner.Score.ToString();
                else msg = "PhoenixCastle Pole has ended with no winner.";
        ...
        internal void ShuffleScores(bool createWinned = false)
        {
            ...
            CitywarPCScrore[] Array;
            Array = ScoreList.Values.ToArray();
            CitywarPCScrore[] DescendingList;
            DescendingList = Array.OrderByDescending((CitywarPCScrore p) => p.Score).ToArray();
            using (RecycledPacket rec = new RecycledPacket())
            {
                ...
                SendMapPacket(new MsgMessage($"-----------------------", MsgMessage.MsgColor.yellow, MsgMessage.ChatMode.ContinueRightCorner).GetArray(stream));
                foreach (var user in Database.Server.ServerMaps[19892].Values)
                {
