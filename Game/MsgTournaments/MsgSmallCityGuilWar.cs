...
{
    public class MsgSmallCityGuilWar
    {
        ...
        public static string Files = "\\SmallCityGuilWar.txt";
        ...
        public MsgSmallCityGuilWar(uint prize)
        {
            currentLocation = -1;
            this.prize = prize;
        ...
        public void GetRound()
        {
            if (!InFight)
            {
                ...
                {
                    client.Player.MessageBox("SmallCity GuilWar began in " + currentMap.Name + ". Would you like to join?", delegate (GameClient p)
                    {
                        joinClient(p);
                    }, null);
                }
                //ServerKernel.Log.SaveLog("PLayer " + lastKiller.Player.Name + " won the pole in " + currentMap.Name + "!The SmallCity GuilWar moved to " + currentMap.Name + "!", true, LogType.DEBUG);
                ...
                using (RecycledPacket recycledPacket2 = new RecycledPacket())
                {
                    ...
                    if (currentLocation != -1 && killedPole && lastKiller != null && currentMap != null)
                        Program.SendGlobalPackets.Enqueue(new MsgMessage("PLayer " + lastKiller.Player.Name + " won the pole in " + currentMap.Name + "!The SmallCity GuilWar moved to " + currentMap.Name + "!", MsgMessage.MsgColor.white, MsgMessage.ChatMode.Guild).GetArray(stream3));
                    else Program.SendGlobalPackets.Enqueue(new MsgMessage("The SmallCity GuilWar moved to " + currentMap.Name + "!", MsgMessage.MsgColor.white, MsgMessage.ChatMode.Guild).GetArray(stream3));
                }
            }
        }
        ...
        {
            if (time64.DayOfWeek != ISchedule.Schedules[ISchedule.EventID.SmallCityGuilWar].DayOfWeek && !ISchedule.Schedules[ISchedule.EventID.SmallCityGuilWar].EveryDay)
                return;
            if (time64.Minute == ISchedule.Schedules[ISchedule.EventID.SmallCityGuilWar].StartMinute)//xx:15
            {
                GetRound();
            }
            if (time64.Minute == ISchedule.Schedules[ISchedule.EventID.SmallCityGuilWar].StartMinute * 2)//xx:30
            {
                GetRound();
            }
        ...
        internal void KillPole(GameClient killer)
        {
            ...
            IEventRewards.Add("SmallCityGuilWar", ServerKernel.POLE_DOMINATION_REWARD , 0, "", "[" + lastKiller.Player.Name + "]: " + DateTime.Now.ToString("d/M/yyyy (H:mm)"));
            ...
            else
            {
                ...
                using (RecycledPacket rec = new RecycledPacket())
                {
                    Packet stream;
                    stream = rec.GetStream();
                    Program.SendGlobalPackets.Enqueue(new MsgMessage("The SmallCity GuilWar finish and Player " + lastKiller.Player.Name + " have taken the pole in " + currentMap.Name + ".", MsgMessage.MsgColor.red, MsgMessage.ChatMode.System).GetArray(stream));
                }
            }
