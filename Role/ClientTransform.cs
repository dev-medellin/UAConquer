...
namespace TheChosenProject.Role
{
    public class ClientTransform
    {
        ...
        {
            if (Owner.Owner.EventBase != null)
            {
                var events = Program.Events.Find(x => x.EventTitle == Owner.Owner.EventBase.EventTitle);
                if (events.InTournament(Owner.Owner))
                {
                    Owner.Owner.SendSysMesage("Item can't be tansform right now.");
                    return;
                }
            }
            HitPoints = hp;
