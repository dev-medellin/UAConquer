...
namespace TheChosenProject.Role.Instance
{
    public class Nobility
    {
        public class NobilityRanking
        {
            ...
            public void RemoveToRank(Player user)
            {
                if (ClientPoll.ContainsKey(user.UID))
                    ClientPoll.Remove(user.UID); // Or TryRemove if using ConcurrentDictionary

                    return; // Prevent re-adding them
        ...
        public NobilityRank Rank
        {
            get
            {
                if (Position >= 0 && Position < 4 || Name.Contains("[KN]"))
