...
namespace TheChosenProject.Role.Instance
{
    public class Spell
    {
        ...
        public void Add(Packet stream, ushort ID, ushort level = 0, byte levelsoul = 0, byte previouslevel = 0, int Experience = 0, bool ClearExp = false)
        {
            if (ID != 1105)
            {
                MsgSpell my_spell;
                my_spell = new MsgSpell
                {
                    ID = ID,
                    Level = level,
