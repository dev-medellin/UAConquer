...
namespace TheChosenProject.Database
{
    public class ConfiscatorTable
    {
        ...
        internal unsafe void Save()
        {
            ...
            if (binary.Open(ServerKernel.CO2FOLDER + "\\RedeemContainer.bin", FileMode.Create))
            {
                int ItemCount;
            ...
            if (binary.Open(ServerKernel.CO2FOLDER + "\\ClaimContainer.bin", FileMode.Create))
            {
                int ItemCount;
        ...
        {
            WindowsAPI.BinaryFile binary = new WindowsAPI.BinaryFile();
            if (binary.Open(ServerKernel.CO2FOLDER + "\\RedeemContainer.bin", FileMode.Open))
            {
            ...
            binary = new WindowsAPI.BinaryFile();
            if (binary.Open(ServerKernel.CO2FOLDER + "\\ClaimContainer.bin", FileMode.Open))
            {
                Game.MsgServer.MsgDetainedItem Item;
                int ItemCount;
                binary.Read(&ItemCount, sizeof(int));
