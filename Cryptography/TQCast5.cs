...
namespace TheChosenProject.Cryptography
{
    public class GameCryptography
    {
        ...
        public void Decrypt(byte[] packet, int length)
        {
            ...
            //  System.Buffer.BlockCopy(buffer, 0, packet, 0, length);
        }
        ...
        {
            byte[] buffers = _blowfish.Decrypt(buffer);
            System.Buffer.BlockCopy(buffers, 0, packet, position, length);
            //        System.Buffer.BlockCopy(buffers, 0, buffer, 0, buffers.Length);
            //System.Buffer.BlockCopy(packet, length, buffer, length, position);
            //byte[] data = new byte[36];
            // Buffer.BlockCopy(buffers, 0, packet, 0, 10);
            // System.Buffer.BlockCopy(buffers, 0, buffer, 0, buffers.Length);
        }
